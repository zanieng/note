...
2. 根以外的非叶子节点（内部节点）有 $\lceil M \rceil$ ~ M个子节点（每个内部节点中的索引值最多只有M-1个，如下图第二层的第二个节点25,31,41，4阶B+树最多只能有3个值），每个叶节点有$\lceil M \rceil$~M个键值（真正存储的元素）
   - B+树真正的元素存储在叶子中
...
!!! 注意
    有的时候如果兄弟节点有空位置的话可以将插入的元素推给兄弟节点
